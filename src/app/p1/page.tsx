...
export default function Player1Page() {
  const playerId = 'player1' as const;
  const { game, isConnected, isNarratorSpeaking, gameOver, won } = useGame();
  const [transcript, setTranscript] = useState('');
  const [isSending, setIsSending] = useState(false);

  const isMyTurn = game?.state === 'PLAYER_1_TURN' || game?.state === 'ASKING_PLAYER1_NAME';
  const isNamePhase = game?.state === 'ASKING_PLAYER1_NAME';
  ...
  if (!game || game.state === 'WAITING_TO_START') {
    return (
      <div className="min-h-screen bg-gray-900 flex items-center justify-center p-4">
        <div className="text-center">
          <h1 className="text-2xl font-serif text-amber-100 mb-4">The Parlor</h1>
          <p className="text-amber-500 mb-2">Player 1</p>
    ...
    <div className="min-h-screen bg-gray-900 text-white p-4 flex flex-col">
      ...
      <header className="text-center mb-6">
        ...
        <p className="text-amber-500/80 text-sm">
          Player 1: {game.players.player1.name || 'Detective'}
        </p>
      </header>
      ...
      <TurnIndicator
        ...
        waitingMessage={
          game.state === 'ASKING_PLAYER2_NAME'
            ? 'Player 2 is introducing themselves'
            : game.state === 'PLAYER_2_TURN'
            ? 'Player 2 is asking a question'
            : undefined
        }
